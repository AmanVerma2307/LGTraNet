...
####### Loading Dataset
X_train = np.array(np.load('./Data/X_MIT_Gallery.npz',allow_pickle=True)['arr_0'],dtype=np.float16)
y_train = np.load('./Data/y_MIT_Gallery.npz',allow_pickle=True)['arr_0']
X_dev = np.array(np.load('./Data/X_MIT_Probe.npz',allow_pickle=True)['arr_0'],dtype=np.float16)
...
arc_logit_layer = ArcFace(113+89,30.0,0.3,tf.keras.regularizers.l2(1e-4))

##### Defining Architecture

#### Input Layer
Inputs = Input_layer
Input_Labels = tf.keras.layers.Input(shape=(47,))
...
model.load_weights('./models/CD_PEM.h5')
#model.summary()
...
np.savez_compressed('./embeddings/EBH_CD_pe-mit.npz',Train_Embeddings)
np.savez_compressed('./embeddings/EBH_CD_pe-mit.npz',Test_Embeddings)
