...
####### Loading Dataset
X_train = np.array(np.load('./Data/X_PTB_Gallery.npz',allow_pickle=True)['arr_0'],dtype=np.float16)
y_train = np.load('./Data/y_PTB_Gallery.npz',allow_pickle=True)['arr_0']
X_dev = np.array(np.load('./Data/X_PTB_Probe.npz',allow_pickle=True)['arr_0'],dtype=np.float16)
y_dev = np.load('./Data/y_PTB_Probe.npz',allow_pickle=True)['arr_0']
...
#### Input Layer
Inputs = Input_layer
Input_Labels = tf.keras.layers.Input(shape=(113,))
...
model.load_weights('./models/EBH_CD-ECG1D.h5')
#model.summary()
...
np.savez_compressed('./embeddings/EBH_CD_ecg1d-ptb_gallery.npz',Train_Embeddings)
np.savez_compressed('./embeddings/EBH_CD_ecg1d-ptb_probe.npz',Test_Embeddings)
